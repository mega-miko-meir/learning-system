...
export default function HRUserForm({ user, departments, managers }) {
    const isEdit = !!user;
    ...
    function submit(e) {
        e.preventDefault();
        if (isEdit) {
            put(route("hr.users.update", user.id));
        } else {
            ...
            <div className="max-w-2xl">
                ...
                {!isEdit && (
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-5 text-sm text-blue-700">
                        После создания система автоматически назначит временный пароль и обучение по матрице должности.
                    </div>
                )}
                ...
                <form onSubmit={submit} className="bg-white rounded-xl border border-gray-100 p-6 space-y-5">
                        ...
                        {[
                            ...
                            <div key={field}>
                                ...
                                <input value={data[field]} onChange={(e) => setData(field, e.target.value)}
                                    className={`w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors[field] ? "border-red-300" : "border-gray-200"}`} />
                    ...
                    <div className="flex gap-3 pt-2">
                        <button type="submit" disabled={processing}
                            ...
                            {processing ? "Сохраняем..." : isEdit ? "Сохранить" : "Добавить сотрудника"}
                        </button>
                        <Link href={isEdit ? route("hr.users.show", user.id) : route("hr.users.index")}
