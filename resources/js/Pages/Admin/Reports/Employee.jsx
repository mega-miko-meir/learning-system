    ...
    return (
        <div className="mt-2 rounded-lg border border-gray-100 overflow-hidden text-xs">
            <div className={`flex items-center gap-3 px-3 py-2 ${attempt.passed ? "bg-green-50" : "bg-red-50"}`}>
                ...
                <span className={`px-1.5 py-0.5 rounded-full text-xs ${attempt.passed ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>
                ...
                <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th className="px-3 py-1.5 text-center font-medium text-gray-500 w-20">Итог</th>
                        </tr>
                    </thead>
...
export default function EmployeeReport({ employee, assignments }) {
    ...
    return (
        <AppLayout title={`Отчёт: ${employee.full_name}`}>
            <Head title={`Отчёт: ${employee.full_name}`} />

            <div className="flex items-center justify-between mb-6">
                <p className="text-xs text-gray-400">
                    <Link href={route("admin.reports.index")} className="hover:underline">← Отчёты</Link>
                </p>
                <a href={route("admin.reports.employee.pdf", employee.id)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                    <div className="bg-white rounded-xl border border-gray-100 p-5">
                        <h2 className="text-sm font-semibold text-gray-700 mb-3">{employee.full_name}</h2>
