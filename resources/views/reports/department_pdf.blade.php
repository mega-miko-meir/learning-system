...
<div class="header">
    ...
    <div class="title">Отчёт по обучению отдела «{{ $department->name }}»</div>
    <div class="subtitle">
        @if($dateFrom || $dateTo)
            Период: {{ $dateFrom ? \Carbon\Carbon::parse($dateFrom)->format('d.m.Y') : 'начало' }}
            &ndash;
            {{ $dateTo ? \Carbon\Carbon::parse($dateTo)->format('d.m.Y') : 'сегодня' }} &nbsp;·&nbsp;
        @endif
...
<div class="section">
    <div class="section-title">Сводная статистика по отделу</div>
    <table width="100%" cellspacing="6">
        <tr>
            ...
            <td style="text-align:center;border:1px solid #e2e8f0;padding:10px;border-radius:6px">
                <div style="font-size:22px;font-weight:bold;color:#1d4ed8">{{ $deptTotal }}</div>
                <div style="font-size:10px;color:#64748b">Всего назначений</div>
            </td>
            <td style="text-align:center;border:1px solid #e2e8f0;padding:10px;border-radius:6px">
                <div style="font-size:22px;font-weight:bold;color:#065f46">{{ $deptCompleted }}</div>
                <div style="font-size:10px;color:#64748b">Выполнено</div>
            </td>
            <td style="text-align:center;border:1px solid #e2e8f0;padding:10px;border-radius:6px">
                <div style="font-size:22px;font-weight:bold;color:#991b1b">{{ $deptOverdue }}</div>
                <div style="font-size:10px;color:#64748b">Просрочено</div>
            </td>
            <td style="text-align:center;border:1px solid #e2e8f0;padding:10px;border-radius:6px">
                <div style="font-size:22px;font-weight:bold;color:#1d4ed8">{{ $deptPercent }}%</div>
