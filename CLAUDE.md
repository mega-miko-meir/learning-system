...
- **Функция за флагом** `FEATURE_INDUCTION` (`config/features.php`, по умолчанию `false`; в `.env` на проде строки нет — выключено). Выключено: маршруты `employee.induction.*` и `admin.materials.*`/`documents.materials.store` отдают 404 (middleware `feature:induction`), в форме и карточке документа скрыты выбор режима и блок материалов, документ с `completion_mode = confirmation` ведёт себя как обычный. Включить: `FEATURE_INDUCTION=true` в `.env` + `php artisan config:clear` (на проде — `config:cache`); пересборка фронта не нужна (флаг приходит в Inertia-пропсе `features.induction`). Миграции таблиц/колонок при выключенной функции остаются (они только добавляют).
